class AppFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def string_input(method, options = {})
    form_group(method, options) do
      string_field(method, merge_input_options({input_html: {class: "input input-bordered bg-transparent border border-gray-300 p-2 rounded-lg #{"input-error" if has_error?(method)}"}}, options))
    end
  end

  def text_input(method, options = {})
    form_group(method, options) do
      text_area(method, merge_input_options({class: "input input-bordered bg-transparent border border-gray-300 p-2 rounded-lg min-h-20 #{"input-error" if has_error?(method)}"}, options[:input_html]))
    end
  end
  ...
  def select_input(method, options = {})
    ...
    collection_input(method, options) do
      collection_select(method, options[:collection], value_method, text_method, options, merge_input_options({class: "select select-bordered bg-transparent border border-gray-300 p-2 rounded-lg #{"is-invalid" if has_error?(method)}"}, options[:input_html]))
    end
  end
